...
These notes intentionally include both the currently smoked release-facing
surface and the broader preparatory work that is already in place or being
hardened, so internal readers can see not only what is fully exercised today
but also what GENtle is being prepared for next.
...
- Gibson specialist baseline is now explicitly documented as the interim
  release-facing cloning specialist:
...
## Validated Commands (earlier release-candidate checks)
...
- `cargo test -q gibson_apply_blocked_status_surfaces_existing_termini_limitation`: pass
- `cargo test -q default_save_file_names_follow_current_project_name`: pass
- `cargo check -q`: pass
...
- More broadly, several areas described above are intentionally included here
  as prepared groundwork or partially hardened capability, not as a claim that
  every path has already received the same end-to-end release testing.
...
5. Run the local release-shaped smoke matrix from [`docs/release.md`](docs/release.md).
6. Confirm the Gibson specialist help/status text still warns that multi-insert
   apply requires a defined destination opening.
7. Verify README/tutorial wording still matches current menu entries:
   - `Patterns -> Gibson...`
