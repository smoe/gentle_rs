...
mod tests {
    ...
    fn hosted_profile_queue_waits_for_local_job_and_blocks_old_view_export() {
        ...
        area.tss_ui.local_scores.job = Some(Arc::new(LocalScoreJob {
            source: source.clone(),
            request: area.tss_ui.local_scores.request(),
            cancel: Arc::new(AtomicBool::new(false)),
            progress: Arc::new(Mutex::new(None)),
            receiver: Mutex::new(receiver),
        }));
        area.queue_tss_profile(path.clone()).unwrap();
        ...
        assert_eq!(area.tss_ui.requested_profile_path.as_ref(), Some(&path));
        assert!(area.tss_svg_snapshot(ViewSvgExportProfile::Screen).is_err());

        // Deterministic worker completion, without executing motif scoring.
        send.send(Err("synthetic scoring failure".into())).unwrap();
        area.poll_tss_local_scores(&ctx);
        assert!(!area.tss_ui.local_scores.running());
        assert!(area.tss_svg_snapshot(ViewSvgExportProfile::Screen).is_err());
        render(&mut area);
        assert!(area.tss_ui.requested_profile_path.is_none());
        let attachment = area.tss_ui.profile_load.as_ref().unwrap();
        assert!(Arc::ptr_eq(&attachment.source, &source));
