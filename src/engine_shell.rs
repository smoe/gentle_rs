...
fn normalize_planning_class_key(raw: &str) -> String {
    GentleEngine::normalize_planning_class_key(raw)
}
    ...
    pub fn preview(&self) -> String {
        match self {
            ...
            Self::RenderProtocolCartoonTemplateWithBindingsSvg {
                ...
                "render protocol cartoon template '{}' with bindings '{}' to '{}'",
                template_path, bindings_path, output
            ),
            ...
            Self::ResourcesSyncJasparRemoteMetadata {
                ...
                let output = output.clone().unwrap_or_else(|| {
                    resource_sync::DEFAULT_JASPAR_REMOTE_METADATA_PATH.to_string()
                });
                format!(
                    "sync JASPAR remote metadata for {}{}{} to '{}'",
                    if motifs.is_empty() {
                        "matching local registry entries".to_string()
                    } else {
                        motifs.join(",")
                    },
                    filter
            ...
            Self::RnaReadsInspectGeneSupport {
                report_id,
                gene_ids,
                selected_record_indices,
                complete_rule,
                ...
                "inspect RNA-read gene-support cohort for '{}' (genes={}, selected_record_indices={}, complete_rule={}, cohort={}, output={})",
                report_id,
                if gene_ids.is_empty() {
                    "<none>".to_string()
                } else {
    ...
    match tokens[1].as_str() {
        ...
        "open" | "focus" => {
            ...
            let target = UiIntentTarget::parse(&tokens[2]).ok_or_else(|| {
                "Unknown ui target. Expected one of: prepared-references, prepare-reference-genome, retrieve-genome-sequence, blast-genome-sequence, import-genome-track, pcr-design, sequencing-confirmation, agent-assistant, prepare-helper-genome, retrieve-helper-sequence, blast-helper-sequence".to_string()
            })?;
            let mut genome_id: Option<String> = None;
        ...
        "protocol-cartoon" => {
            ...
            match tokens[1].as_str() {
                ...
                "template-validate" => {
                    ...
                    Ok(ShellCommand::ValidateProtocolCartoonTemplate {
                        template_path: tokens[2].clone(),
                    })
                }
                "render-with-bindings" => {
                    ...
                    let bindings_path = tokens[3].trim();
                    ...
                    Ok(ShellCommand::RenderProtocolCartoonTemplateWithBindingsSvg {
                        template_path: tokens[2].clone(),
                        bindings_path: tokens[3].clone(),
                        output: tokens[4].clone(),
                    })
                }
                "template-export" => {
                    if tokens.len() != 4 {
                ...
                "sync-jaspar-remote-metadata" => {
                    ...
                    while idx < tokens.len() {
                        match tokens[idx].as_str() {
                            ...
                            other => {
                                return Err(format!(
                                    "Unknown option '{other}' for resources sync-jaspar-remote-metadata"
                                ));
                            }
                        }
                    }
                    ...
                    Ok(ShellCommand::ResourcesSyncJasparRemoteMetadata {
                        motifs,
                        filter,
                        limit,
                        output,
    ...
    match command {
        ...
        ShellCommand::ResourcesSyncJasparRemoteMetadata {
            ...
            let resolved_output = output
                .clone()
                .unwrap_or_else(|| resource_sync::DEFAULT_JASPAR_REMOTE_METADATA_PATH.to_string());
            let op_result = engine
...
fn format_construct_reasoning_fact_summary(fact: &crate::engine::DesignFact) -> Option<Value> {
    ...
    match fact.fact_type.as_str() {
        ...
        "cloning_stability_context" => {
            ...
            if inversion > 0 {
                warning_lines.push(
                    "Inverted-repeat evidence suggests inversion/recombination review".to_string(),
                );
            }
        }
        _ => {}
