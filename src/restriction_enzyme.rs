...
impl RestrictionEnzyme {
    ...
    pub fn get_sites(&self, seq: &DNAsequence, max_sites: Option<usize>) -> Vec<RestrictionEnzymeSite> {
        ...
        for start in 0..seq_len {
            let range = std::ops::Range {start, end: start+recognition_len};
            let s = forward.get(range); // TODO circular
            match s {
                Some(s) => {
                    let s = std::str::from_utf8(s).unwrap();
                    if s==self.sequence { // TODO IUPAC
                        ret.push(RestrictionEnzymeSite { offset: start, enzyme: self.to_owned() , forward_strand:true });
                    }
                },
                None => {},
            }
