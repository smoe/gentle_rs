...
impl RenderCircularMapSVG {
    ...
    pub fn from_dna_sequence(dna: &DNAsequence) -> Self {
        ...
        let label = svg::node::element::Text::new(dna.name().clone().unwrap_or_default())
            .set("x", 2000)
            .set("y", 1500)
        ...
        let size_text = svg::node::element::Text::new(format!("({} bp)", dna.len()))
            .set("x", 2000)
            .set("y", 1600)
        ...
        for feature in dna.features() {
            match feature.location {
                gb_io::seq::Location::Range(from, to) => {
                    ret.document =
                        Self::render_feature_from_range(&mut ret, feature, max_pos, from, to);
                }
    ...
    fn render_feature_from_range(
        ret: &RenderCircularMapSVG,
        feature: &Feature,
        max_pos: i64,
        ...
        let from = start.0;
        let to = end.0;
        let inner = 1000.0 + (Self::feature_band(feature) as f64) * 100.0 - FEATURE_HEIGHT / 2.0
            + FEATURE_HEIGHT / 10.0;
        ...
        let label = svg::node::element::Text::new(label_text)
            .set("x", x)
            .set("y", y)
            .set("font-size", FONT_SIZE_SMALL)
            .set("font-family", "Verdana")
