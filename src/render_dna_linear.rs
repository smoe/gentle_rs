...
use crate::{
    ...
    linear_base_routing::{
        ...
        LinearBaseRenderMode as RoutingBaseRenderMode, LinearBaseRoutingDecision,
...
const SEQUENCE_BASE_FEATURE_CLEARANCE_PADDING: f32 = 8.0;
#[cfg(test)]
const CONDENSED_FEATURE_CLEARANCE_PADDING: f32 = SEQUENCE_BASE_FEATURE_CLEARANCE_PADDING;
...
impl RenderDnaLinear {
    ...
    fn sequence_base_render_status(&self, viewport: LinearViewport) -> SequenceBaseRenderStatus {
        ...
        let mut routing = decide_linear_base_routing(LinearBaseRoutingInput {
        ...
        if !sequence_bases_enabled {
            routing.active_mode = RoutingBaseRenderMode::Off;
            routing.decision_reason =
    ...
    fn sequence_base_text_half_extent(&self, viewport: LinearViewport) -> Option<f32> {
        let status = self.sequence_base_render_status(viewport);
        if status.active_mode == SequenceBaseRenderMode::Off {
            return None;
        }
        ...
        let mode_extra = match status.active_mode {
            SequenceBaseRenderMode::Off => return None,
            SequenceBaseRenderMode::StandardLinear => 0.0,
            SequenceBaseRenderMode::ContinuousHelical => font_size,
            SequenceBaseRenderMode::Condensed10Row => {
                let row_step = Self::condensed_row_step(font_size, show_double_strand);
                row_step * CONDENSED_HELICAL_ROW_COUNT.saturating_sub(1) as f32
            }
        };
        Some(font_size + strand_half_gap + mode_extra)
    }
    ...
    fn condensed_helical_text_half_extent(&self, viewport: LinearViewport) -> Option<f32> {
        ...
        self.sequence_base_text_half_extent(viewport)
    }
    ...
    fn layout_features(&mut self, viewport: LinearViewport) {
        ...
        let sequence_text_half_extent = self.sequence_base_text_half_extent(viewport);
        let sequence_text_band_total = sequence_text_half_extent.unwrap_or(0.0) * 2.0;
        let available_total = (self.area.height()
            - BASELINE_TOP_PADDING
            - BASELINE_BOTTOM_PADDING
            - sequence_text_band_total)
            .max(40.0);
        ...
        let reserved_half = sequence_text_half_extent.unwrap_or(0.0);
        ...
        let sequence_feature_clearance = self
            .sequence_base_text_half_extent(viewport)
            .map(|half_extent| half_extent + SEQUENCE_BASE_FEATURE_CLEARANCE_PADDING);
        if let Some(min_clearance) = sequence_feature_clearance {
        ...
        let regulatory_near_baseline_margin = sequence_feature_clearance
            .map(|min_clearance| {
    ...
    fn draw_sequence_bases(&self, painter: &egui::Painter, viewport: LinearViewport) {
        ...
        for (offset, base) in seq.iter().enumerate() {
            ...
            let x2_projected = Self::sequence_base_boundary_x(
                &self.area,
                viewport,
                bp.saturating_add(1),
                render_status.active_mode,
            ...
            let mut x_center = (x1_projected + x2_projected) * 0.5;
            let (forward_y_bp, reverse_y_bp) = if use_condensed_helical {
                let row_from_bottom =
                ...
                Self::continuous_helical_phase_pair(
                    forward_y,
                    reverse_y,
                    helical_phase,
                    helical_parallel_strands,
    ...
    fn helical_projection_column_count(span_bp: usize, progress: f32) -> usize {
        if span_bp == 0 {
            return 0;
        }
        let progress = progress.clamp(0.0, 1.0);
        let column_ratio = 1.0 - (1.0 - HELICAL_MIN_COLUMN_RATIO) * progress;
        (span_bp as f32 * column_ratio)
            .ceil()
            .max(1.0)
    ...
    fn sequence_base_boundary_x(
        ...
        viewport: LinearViewport,
        boundary_bp: usize,
        render_mode: SequenceBaseRenderMode,
        helical_t: f32,
    ) -> f32 {
        if viewport.span == 0 {
            return area.left();
        }
        let bp_offset = boundary_bp
            .saturating_sub(viewport.start)
            .min(viewport.span);
        if render_mode != SequenceBaseRenderMode::ContinuousHelical {
            let frac = bp_offset as f32 / viewport.span as f32;
            return area.left() + frac * area.width();
        }
        let columns = Self::helical_projection_column_count(viewport.span, helical_t);
        if columns == 0 {
            return area.left();
        }
    ...
    fn continuous_helical_phase_index(bp: usize, offset_bp: usize) -> usize {
        bp.wrapping_add(offset_bp) % CONDENSED_HELICAL_ROW_COUNT
    }

    fn continuous_helical_phase_angle(phase_index: usize) -> f32 {
        (phase_index as f32 / CONDENSED_HELICAL_ROW_COUNT as f32) * std::f32::consts::TAU
    }
    ...
    fn continuous_helical_phase_pair(
        ...
        reverse_y: f32,
        helical_phase: f32,
        parallel_strands: bool,
    ) -> (f32, f32) {
        let reverse_phase = if parallel_strands {
            helical_phase
        } else {
            -helical_phase
        };
        (forward_y + helical_phase, reverse_y + reverse_phase)
    }

    fn continuous_helical_phase(
        phase_index: usize,
        phase_angle: f32,
        font_size: f32,
        helical_t: f32,
        ...
        // Keep a true oscillating helix for most of the continuous range, then
        // blend late toward a 2-row split as density nears 2x capacity.
        ...
        let blend_to_two_row = ((helical_t.clamp(0.0, 1.0) - HELICAL_TWO_ROW_BLEND_START)
            / (1.0 - HELICAL_TWO_ROW_BLEND_START))
            .clamp(0.0, 1.0)
            .powf(1.25);
        sine_component * (1.0 - blend_to_two_row) + two_row_component * blend_to_two_row
    }
    ...
    fn draw_name_and_length(&self, painter: &egui::Painter, viewport: LinearViewport) {
        ...
        self.draw_array_color_legend(painter);
        painter.text(
        ...
        painter.text(
            ...
            Align2::RIGHT_TOP,
            format!(
                "route: {} | setting: {} | letters={} | compressed={} | parallel={} | rev-alpha={:.2} | hide-pref={} | backbone={}",
                status.route_policy_label(),
                status.mode_setting_label(),
                status.sequence_bases_enabled,
    ...
    fn draw_array_color_legend(&self, painter: &egui::Painter) {
        if !self.features.iter().any(|feature| feature.is_array_track) {
            return;
        }
        let origin = Pos2::new(self.area.left() + 6.0, self.area.top() + 26.0);
        let panel = Rect::from_min_size(origin, Vec2::new(422.0, 32.0));
        painter.rect_filled(
            panel,
            4.0,
        ...
        painter.rect_stroke(
            panel,
            4.0,
            Stroke::new(0.6_f32, Color32::from_rgba_unmultiplied(80, 80, 80, 70)),
            StrokeKind::Outside,
        );
        painter.text(
            origin + Vec2::new(7.0, 5.0),
            Align2::LEFT_TOP,
            "Array: logFC",
            FontId {
                size: 9.5,
        ...
        let swatches = [
            ...
            (Color32::from_rgb(150, 150, 150), "~0"),
            (Color32::from_rgb(37, 99, 235), "-"),
        ];
        let mut x = origin.x + 92.0;
        for (color, label) in swatches {
            let rect = Rect::from_min_size(Pos2::new(x, origin.y + 7.0), Vec2::new(15.0, 10.0));
            painter.rect_filled(rect, 1.5, color);
            painter.text(
                Pos2::new(x + 19.0, origin.y + 4.0),
                Align2::LEFT_TOP,
                label,
                FontId {
                    size: 9.0,
            ...
            x += if label == "~0" { 45.0 } else { 32.0 };
        }
        painter.text(
            Pos2::new(origin.x + 218.0, origin.y + 5.0),
            Align2::LEFT_TOP,
            "darker/outlined = stronger or adj.P.Val<=0.05",
            FontId {
                size: 9.0,
                family: FontFamily::Monospace,
            },
...
mod tests {
            ...
            display.set_construct_reasoning_overlay(Some(ConstructReasoningOverlay {
                ...
                evidence: vec![
                    ConstructReasoningOverlaySpan {
                        ...
                        summary_id: "summary_forward_promoter".to_string(),
                    ...
                    ConstructReasoningOverlaySpan {
                        ...
                        summary_id: "summary_reverse_tfbs".to_string(),
                        summary_title: "TFBS".to_string(),
                        summary_subtitle: "TFBS candidate".to_string(),
                        summary_candidate_count: 1,
                        summary_review_status: "draft".to_string(),
                        start_0based: 360,
            ...
            display.set_construct_reasoning_overlay(Some(ConstructReasoningOverlay {
                ...
                evidence: vec![
                    ...
                    ConstructReasoningOverlaySpan {
                        ...
                        summary_id: "summary_tfbs".to_string(),
                        summary_title: "TFBS".to_string(),
                        summary_subtitle: "TFBS candidate".to_string(),
                        summary_candidate_count: 1,
                        summary_review_status: "draft".to_string(),
    ...
    #[test]
    fn sequence_base_track_visibility_flag_overrides_forced_modes() {
        let renderer = test_renderer(5000);
        {
            let mut display = renderer.display.write().expect("display");
        ...
        let status = renderer.sequence_base_render_status(viewport);
    ...
    #[test]
    fn continuous_helical_projection_keeps_endpoints_at_viewport_edges() {
        let area = Rect::from_min_max(Pos2::new(100.0, 50.0), Pos2::new(900.0, 250.0));
        let viewport = LinearViewport {
            start: 1000,
            end: 1400,
            span: 400,
        ...
        assert!((left - area.left()).abs() < 0.001);
        assert!((right - area.right()).abs() < 0.001);
    }

    #[test]
    fn continuous_helical_projection_uses_fewer_columns_than_standard() {
        let area = Rect::from_min_max(Pos2::new(0.0, 0.0), Pos2::new(1000.0, 200.0));
        let viewport = LinearViewport {
            start: 0,
            end: 100,
            span: 100,
        };
        let x_std_40 = RenderDnaLinear::sequence_base_boundary_x(
            &area,
            viewport,
            40,
            SequenceBaseRenderMode::StandardLinear,
        ...
        let x_hel_40 = RenderDnaLinear::sequence_base_boundary_x(
            &area,
            viewport,
            40,
            SequenceBaseRenderMode::ContinuousHelical,
        ...
        let x_std_41 = RenderDnaLinear::sequence_base_boundary_x(
            &area,
            viewport,
            41,
            SequenceBaseRenderMode::StandardLinear,
        ...
        let x_hel_41 = RenderDnaLinear::sequence_base_boundary_x(
            &area,
            viewport,
            41,
            SequenceBaseRenderMode::ContinuousHelical,
        ...
        assert!(x_hel_40 <= x_std_40);
        assert!((x_hel_41 - x_hel_40) <= (x_std_41 - x_std_40));
    }
    ...
    fn continuous_helical_phase_pair_supports_parallel_or_mirrored_strands() {
        ...
        assert!((f_parallel - 12.5).abs() < 0.001);
        assert!((r_parallel - 22.5).abs() < 0.001);

        let (f_mirrored, r_mirrored) =
            RenderDnaLinear::continuous_helical_phase_pair(10.0, 20.0, 2.5, false);
        assert!((f_mirrored - 12.5).abs() < 0.001);
        assert!((r_mirrored - 17.5).abs() < 0.001);
    }
    ...
    fn continuous_helical_phase_oscillates_and_grows_with_density() {
        ...
        let high =
            RenderDnaLinear::continuous_helical_phase(high_index, high_angle, 12.0, 1.0, 0.8);
        let high_alt_index = RenderDnaLinear::continuous_helical_phase_index(2, 0);
        let high_alt_angle = RenderDnaLinear::continuous_helical_phase_angle(high_alt_index);
        let high_alt = RenderDnaLinear::continuous_helical_phase(
            high_alt_index,
            high_alt_angle,
            12.0,
            1.0,
        ...
        assert!(high.signum() != high_alt.signum());
    }

    #[test]
    fn continuous_helical_phase_is_not_monotonic_ramp() {
        let values: Vec<f32> = (0..CONDENSED_HELICAL_ROW_COUNT)
            .map(|idx| {
                let angle = RenderDnaLinear::continuous_helical_phase_angle(idx);
                RenderDnaLinear::continuous_helical_phase(idx, angle, 12.0, 0.45, 0.45)
            })
            .collect();
        let monotonic_non_decreasing = values.windows(2).all(|pair| pair[1] >= pair[0]);
        let monotonic_non_increasing = values.windows(2).all(|pair| pair[1] <= pair[0]);
        assert!(!monotonic_non_decreasing);
        assert!(!monotonic_non_increasing);
    ...
    #[test]
    fn condensed_helical_rows_scale_step_with_font_size() {
        let row_step = RenderDnaLinear::condensed_row_step(SEQUENCE_BASE_TEXT_MAX_FONT_SIZE, true);
        assert!(row_step >= SEQUENCE_BASE_TEXT_MAX_FONT_SIZE * CONDENSED_HELICAL_ROW_SPACING_SCALE);
    }
    ...
    #[test]
    fn helical_base_letters_reserve_clearance_for_promoter_lane() {
        let feature = make_test_feature_with_kind("promoter", Location::simple_range(120, 260));
        ...
        let half_extent = renderer
            .sequence_base_text_half_extent(viewport)
            .expect("helical sequence base extent");
        let clearance = renderer.baseline_y() - renderer.features[0].rect.bottom();
        assert!(
            clearance >= half_extent + SEQUENCE_BASE_FEATURE_CLEARANCE_PADDING - 0.5,
            "feature clearance {clearance:.2} should reserve base-letter half extent {half_extent:.2}"
        );
    }
    ...
    fn condensed_layout_keeps_reverse_strand_ordered_below_forward_strand() {
        ...
        let mut previous_forward: Option<f32> = None;
        let mut previous_reverse: Option<f32> = None;
        for row_from_top in 0..CONDENSED_HELICAL_ROW_COUNT {
            let row_offset = row_from_top as f32 * row_step;
            let forward = forward_y - row_offset;
            let reverse = reverse_y + row_offset;
            assert!(reverse > forward);
            if row_from_top == 0 {
                assert!(
                    (reverse - forward - (font_size + CONDENSED_HELICAL_STRAND_GAP)).abs() < 0.001
                );
            }
            if let Some(prev_forward) = previous_forward {
                assert!((prev_forward - forward - row_step).abs() < 0.001);
            }
            if let Some(prev_reverse) = previous_reverse {
                assert!((reverse - prev_reverse - row_step).abs() < 0.001);
            }
            previous_forward = Some(forward);
            previous_reverse = Some(reverse);
        }
    }
    ...
    fn condensed_layout_single_strand_reserves_full_text_band() {
        ...
        let half_extent = renderer
            .condensed_helical_text_half_extent(viewport)
            .expect("condensed extent");
        let minimum_half_extent = (CONDENSED_HELICAL_ROW_COUNT.saturating_sub(1) as f32
            * CONDENSED_HELICAL_SINGLE_ROW_MIN_STEP)
            + SEQUENCE_BASE_TEXT_MIN_FONT_SIZE;
