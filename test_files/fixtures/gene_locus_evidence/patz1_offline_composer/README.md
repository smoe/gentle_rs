...
condition-wise abundance means plus TAp73alpha-GFP, DNp73beta-GFP, and
TAp73alpha-DNp73beta effects. The four BED files contain one invented occupancy
interval each and exist only to exercise grouped, shared-scale locus rendering.
...
3. Place `JUC_SYN_PATZ1_SKIP` across junction edges
   `31325860..31325979`.
4. Retain the exact abundance/effect values and BED intervals committed here.
   No random values, timestamps, network results, or local analysis outputs
   enter the fixture.
...
- Workflow/example tests exercise the shared `RenderFeatureExpertSvg`
  operation. The SVG must retain separate PSR/JUC markers, abundance and
  differential lanes, grouped occupancy lanes, and TP73 motif provenance.
