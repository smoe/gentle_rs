...
| `mini_reads.fastq` / `.fastq.gz` | Five reads: `readA_geneA` + `readA2_geneA` (GENEA only), `readB_geneB` (GENEB only), `readAB_ambiguous` (spans GENEA and GENEB), `readN_nohit` (matches nothing). |
