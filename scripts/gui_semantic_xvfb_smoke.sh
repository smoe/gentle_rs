...
click_item() {
  ...
  point=$(jq -r --arg id "$semantic_id" --arg scope "$scope" \
    "$selector | [((.rect_logical_points.min_x + .rect_logical_points.max_x) / 2 * .pixels_per_point | floor), ((.rect_logical_points.min_y + .rect_logical_points.max_y) / 2 * .pixels_per_point | floor)] | @tsv" \
    "$snapshot" | head -1)
  [[ -n "$point" ]] || { echo "FAIL: missing semantic item $semantic_id $scope" >&2; exit 1; }
  read -r x y <<<"$point"
