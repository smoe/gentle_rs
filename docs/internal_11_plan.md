...
| P1: desktop packages | The current recipe stages five entrypoints and tracked resources on Windows/macOS/Linux, using Cargo-default release settings without embedded JS/Lua. Offline regressions cover layouts, archive round-trips, relocation and failures; the reduced recipe needs fresh CI. Native CI must still run `gentle_cli capabilities`, MCP and tutorial-manifest validation from the actual packages; receipts and live GUI acceptance remain required. |
