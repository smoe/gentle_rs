...
- `04.02` [Determine and review PCR primer pairs (offline)](./generated/chapters/04-02_pcr_selection_batch_primer_pairs_offline.md) - executable reference; status `generated+checked/human-pending`; review `codex_reviewed` (2026-07-28) - [file feedback](../../.github/ISSUE_TEMPLATE/tutorial-confusion.md). Use this when: You want a reproducible primer-pair determination example with retained ranked reports, one clean top candidate, and one explicit heuristic advisory.
...
- `08.13` [Explain a PWM/PSSM score versus binding affinity: TP73 and TSS promoter traces](./08-13_motif_logo_to_promoter_trace.md) - guided GUI/CLI; status `manual/hybrid`; review `codex_reviewed` - [file feedback](../../.github/ISSUE_TEMPLATE/tutorial-confusion.md). Use this when: Explain PWM/PSSM and JASPAR TFBS scores, pseudocounts, binding-affinity limits, the corrected TP73 tail and shared-scale TSS promoter traces. Opens a teaching guide without a loaded project; replay needs a built CLI.
- `08.14` [Explore promoter cofactors: from TA/DN associations to a saved motif region](./08-14_promoter_cofactor_browser.md) - guided GUI/CLI; status `manual/hybrid`; review `codex_reviewed` (2026-09-15); human/live GUI review pending. Use this when: Inspect a package without a genome, compare cohort odds ratios and anchor frequencies, interpret retained strand scores and sample/control support, and preserve a selected site as a report-bound genomic region. Includes an offline shared-CLI replay and optional local IRF9 walkthrough.
- `08.15` [From annotated transcript starts to reusable TSS windows](./08-15_tss_collection_gui.md) - offline synthetic GUI/CLI walkthrough with an [executable reference](./generated/chapters/08-15_tss_collection_gui.md); Codex source review 2026-09-19; human/live GUI review pending. Use this when: Preview and approve starts, validate members, open windows repeatedly, then cancel/confirm forgetting and undo. The 18-step automated subset and manual restart/stale-member checklist have separate verdicts.
- `08.16` [Inspect TFBS and regulatory evidence at one annotated TSS](./08-16_tss_regulatory_view_gui.md) - offline synthetic native-GUI walkthrough with retained public inputs and revision-bound screenshots. Use this after 08.15 when you want to distinguish exon/CDS context, supplied CUT&RUN/chromatin signal, stored motif peaks, report score curves, imported peaks and other annotations without turning any of them into a binding or promoter-activity claim. GUI, GUI Shell and inner-agent examples invoke the same shared operation.
