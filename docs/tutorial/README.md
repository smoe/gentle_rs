...
- `08.13` [From a motif logo to a promoter trace: what does the score mean?](./08-13_motif_logo_to_promoter_trace.md) - guided GUI/CLI; status `manual/hybrid`; review `unreviewed` - [file feedback](../../.github/ISSUE_TEMPLATE/tutorial-confusion.md). Use this when: Illustrated, executable explanation of PFM/PWM scores, scanner-native evidence, GENtle dense TSS traces, inclusive background tails, display scaling and reduced promoter-cofactor summaries.
