...
3. Run explicit macOS/Linux/Windows CI (including all-feature checks), headless container validation and [build-only packaging](release.md#build-only-candidate-verification) at one frozen SHA on the approved fork. Check the logged candidate SHA, not only the workflow revision; retain receipts before approval. Native GUI launch/tutorial navigation remain separate. The tagged `.10` Windows ZIP recipe remains GUI-only. No further feature scope until these gates; tagging and upload remain owner-managed.
