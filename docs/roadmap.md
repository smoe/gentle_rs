...
- Release-installer CI now intentionally avoids caching the `target/`
  ...
  - the workflow now triggers automatically from GitHub `release.published`
    ...
    installers instead of relying on a same-moment tag-push event
...
- Multi-crate destination is now documented in
  ...
  - Phase 1 is now initiated:
    ...
    - the shell-first extraction has also promoted several `ShellCommand`
...
- Primer-design report baseline:
  ...
  - cDNA PCR/qPCR assay-test transcript-map SVGs now render group/source exon
    labels (`E1`, `E2`, ...) in transcript-strand order, source-exon identity
...
- UniProt/SWISS-PROT import + genome-projection baseline:
  ...
  - protein/protease follow-up now started:
    ...
    - the Protein Expert is now transcript-first:
      ...
      - the engine now routes external protein evidence through one
...
- Build visual benchmark fixtures (sparse, dense annotations, dense RE sites,
  ...
  - Follow-on (2026-05-01): visual benchmark tests now run a shared structural
