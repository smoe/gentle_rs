...
Last session: 2026-09-17 - TSS collection review regressions addressed; real-data/final-SHA acceptance remains pending.
...
1. Have Glen recheck three introductory GUI chapters plus promoter-similarity navigation and
   TATA evidence review/materialization at the merged SHA, retaining outcomes/screenshots. Exercise the [TSS collection workflow](tss_workspace.md#acceptance) on TP73: separate-process approval, cache refresh, DeltaNp73-only cropped locus, validated scans, repeated opening and inner-agent handoff.
